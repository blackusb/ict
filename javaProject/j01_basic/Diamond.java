...
public class Diamond {
    ...
    //다이아몬드 만들기
    void diamondPaint(int size){    //메소드가 다르면 변수명이 같아도 다르다. startDiamond()에서 size값을 넘겨 받았을뿐.
        char alpha = 'A';
        int step = 2;
        //삼각형
        for(int i=1; i>=1; i+=step){    //i=1,3,5,7,9,11...
            //공백출력
...
//        }
    ...
    public static void main(String[] args) {    //컴파일 하려면 메인메소드가 있어야함
        Diamond diamond = new Diamond();    //내가 원하는 기능이 있는 클래스를 객체로 만들기. 2번 생성자 메소드 실행.
        diamond.startDiamond(); //내가 필요한 메소드를 호출. 객체명.메소드명 -> 메소드 호출
        //시작, 종료는 메인메소드에서.
    }
}
